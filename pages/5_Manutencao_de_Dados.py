...
# --- Lógica da Página ---
st.warning("Atenção: As operações nesta página modificam permanentemente o banco de dados.", icon="⚠️")
...
with col_main:
    ...
    with st.container(border=True):
        st.subheader("Apagar Dados de uma Semana Inteira")
        if not available_weeks:
            ...
            if week_to_delete:
                st.error(
                    f"Você está prestes a apagar todos os registros da Semana {week_to_delete}. Esta ação não pode ser desfeita.", icon="🚨")
                if st.checkbox(f"Confirmo que desejo apagar os dados da Semana {week_to_delete}."):
                    if st.button("Apagar Semana", type="primary", use_container_width=True):
    ...
    with st.container(border=True):
        st.subheader("Sobrescrever Dados de uma Semana")
        st.caption(
        ...
        csv_file = st.file_uploader("Selecione o arquivo CSV para a carga", type="csv")

        if csv_file and week_to_overwrite:
            if st.button(f"Sobrescrever Semana {week_to_overwrite}", type="primary", use_container_width=True):
